...
class Login extends Component {
  enterEventListener(event) {
    const { keyCode } = event
    if (keyCode === 13) {
      const { user, login: _login } = this.props
      _login(user.email.value, user.password.value)
    }
  }
  ...
  render() {
    const { classes, user, setProperty, login: _login } = this.props
    ...
    return (
      <Paper className={s.container} elevation={1} onKeyUp={(e) => this.enterEventListener(e)}>
        ...
        <Button classes={{ contained: classes.button }} color="primary" size="large" variant="contained"
          onClick={() => _login(user.email.value, user.password.value)}>
          Iniciar sessão
        </Button>
      </Paper>
    )
